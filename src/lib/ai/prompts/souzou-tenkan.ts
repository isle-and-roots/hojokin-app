...
情報が不足している箇所は [要入力: ○○] と明示してください。`,
  ...
  company_overview: (profile, context) =>
    `あなたは中小企業診断士として、事業再構築補助金の事業計画書の「企業概要」セクションを作成してください。
...
${context ? `## 追加情報\n${context}\n` : ""}
## 作成ルール
1. 会社の設立年・事業内容・主要製品・サービスを簡潔にまとめる
2. 組織体制（従業員数、主要部門、経営陣の略歴）を示す
3. 主な取引先・顧客層を明記し、事業基盤の安定性を示す
4. 直近の売上・利益などの業績を具体的な数値で記述する
5. 既存事業で培った技術力・ノウハウ・経営資源を強調する
6. 500〜800字程度で記述する

## 審査のポイント
事業再構築補助金では「既存事業の基盤がしっかりしていること」が前提です。審査員が「この事業者なら新事業にも取り組める」と判断できるよう、既存事業の実績と経営資源を具体的に示してください。

## 出力形式
セクション本文をそのまま出力してください（マークダウン書式は使用しない、プレーンテキスト）。
情報が不足している箇所は [要入力: ○○] と明示してください。`,

  restructuring_necessity: (profile, context) =>
    `あなたは中小企業診断士として、事業再構築補助金の事業計画書の「事業再構築の必要性」セクションを作成してください。
...
${context ? `## 追加情報\n${context}\n` : ""}
## 作成ルール
1. 現在の事業環境の変化（市場縮小、競争激化、コロナ影響、デジタル化の波等）を客観的データで示す
2. 売上・利益の推移を具体的な数値で記述し、事業の課題を明確にする
3. 既存事業の構造的な課題を分析し、現状維持では解決できない理由を論理的に説明する
4. 業界全体のトレンドと自社への影響を分析し、事業再構築の必要性を強調する
5. 事業再構築に取り組まなかった場合の将来リスクを示す
6. 600〜1000字程度で記述する
...
情報が不足している箇所は [要入力: ○○] と明示してください。`,
  ...
  implementation_schedule: (profile, context) =>
    `あなたは中小企業診断士として、事業再構築補助金の事業計画書の「実施体制とスケジュール」セクションを作成してください。
...
${context ? `## 追加情報\n${context}\n` : ""}
## 作成ルール
1. 事業の実施体制を明確にする（責任者、担当者、外部連携先・支援機関の役割）
2. 月単位のスケジュールを示す（設備発注→納品→設置→試運転→本稼働→事業開始）
3. 各工程でのマイルストーンと達成基準を記載する
4. 認定経営革新等支援機関との連携体制を明示する
5. リスク要因と対策を1〜2点示す
6. 500〜800字程度で記述する

## 審査のポイント
「実現可能性」の評価では、実施体制の妥当性とスケジュールの具体性が問われます。補助事業期間内に確実に完了できる計画であること、かつ支援機関との連携が明確であることを示してください。
...
情報が不足している箇所は [要入力: ○○] と明示してください。`,
  ...
  revenue_plan: (profile, context) =>
    `あなたは中小企業診断士として、事業再構築補助金の事業計画書の「収益計画」セクションを作成してください。
...
${context ? `## 追加情報\n${context}\n` : ""}
## 作成ルール
1. 補助事業終了後3〜5年の売上・利益の見込みを年度別に示す
2. 付加価値額の年率3%以上増加の達成見込みを具体的な数値で示す
3. 売上見込みの根拠（顧客数×単価、市場シェア等）を明示する
4. 補助金の使途内訳と自己資金・借入等の資金調達計画を示す
5. 投資回収の見込み時期を明記する
6. 400〜600字程度で記述する

## 審査のポイント
数値計画の実現可能性が重視されます。楽観的すぎない堅実な計画を示してください。付加価値額の増加目標（年率3%以上）は必須要件です。

## 出力形式
セクション本文をそのまま出力してください（マークダウン書式は使用しない、プレーンテキスト）。
情報が不足している箇所は [要入力: ○○] と明示してください。`,

  current_business: (profile, context) =>
    `あなたは中小企業診断士として、事業再構築補助金の事業計画書の「現在の事業概要と課題」セクションを作成してください。
...
${context ? `## 追加情報\n${context}\n` : ""}
## 作成ルール
1. 現在の主力事業・製品・サービスの内容を具体的に記述する
2. 売上・利益の推移を直近3〜5年の数値で示す（あれば）
3. 現在の事業における構造的な課題（市場縮小、競争激化、コスト増等）を分析する
4. 現状維持では事業継続が困難な理由を論理的に説明する
5. 既存事業で培った経営資源（技術力、人材、顧客基盤等）を示す
6. 500〜800字程度で記述する

## 審査のポイント
事業再構築の前提となる「現在の事業の実態」を正確に把握していることを示してください。課題が明確であるほど、事業再構築の必要性が説得力を持ちます。

## 出力形式
セクション本文をそのまま出力してください（マークダウン書式は使用しない、プレーンテキスト）。
情報が不足している箇所は [要入力: ○○] と明示してください。`,

  investment_detail: (profile, context) =>
    `あなたは中小企業診断士として、事業再構築補助金の事業計画書の「投資計画と資金調達」セクションを作成してください。
...
${context ? `## 追加情報\n${context}\n` : ""}
## 作成ルール
1. 補助対象となる設備・建物・システム等の投資内容を具体的に記述する（名称・仕様・用途）
2. 投資総額の内訳（建物費、機械装置費、システム構築費等）を明記する
3. 補助金額・自己負担額・借入額等の資金調達計画を示す
4. 各設備の選定理由（なぜその設備が事業再構築に必要か）を説明する
5. 見積取得状況と発注先候補に触れる
6. 600〜800字程度で記述する

## 審査のポイント
投資の必要性と妥当性が審査されます。「なぜその設備が必要か」「なぜその金額か」を具体的に示し、過大投資でないことを説明してください。
