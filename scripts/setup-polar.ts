/**
 * Polar.sh Sandbox セットアップスクリプト
 *
 * 前提: .env.local に POLAR_ACCESS_TOKEN が設定済み
 *
 * 実行: npx tsx scripts/setup-polar.ts
 *
 * 処理:
 *   1. Polar SDK 初期化 (sandbox)
 *   2. Products 3つ作成（冪等: 同名があればスキップ）
 *   3. Webhook エンドポイント作成（冪等: 同URL があればスキップ）
 *   4. .env.local を実際の値で更新
 */
...
async function main() {
  ...
  console.log("Polar SDK を初期化中 (sandbox)...");
  ...
  try {
    ...
    console.log("\n以下の値を手動で設定してください:");
    printEnvValues(productIds, webhookSecret);
    return;
  }
  ...
  // 5. サマリー
  console.log("\n========================================");
  console.log("Polar Sandbox セットアップ完了!");
  console.log("========================================\n");
  printEnvValues(productIds, webhookSecret);
...
function printEnvValues(
  ...
  console.log("  POLAR_MODE=sandbox");
}
